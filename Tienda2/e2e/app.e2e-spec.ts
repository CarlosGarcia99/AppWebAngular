...
describe('Starting tests for Tienda2', function() {
  ...
  it('website title should be Tienda2', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('Tienda2');
    })
  });
  ...
  it('navbar-brand should be Tienda2',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('Tienda2');
    });
  });
